...
function setup() {
  //establecer tamaño del canvas como 380,380
...
function start()
{
  //crear variable objectDetector como ml5.objectDetector('cocossd', modelLoaded);
...
function gotResult(error, results) {
  ...
  console.log(results);
  //establecer variable objects como results
  objects = results;
}
  ...
  image(video, 0, 0, 380, 380);
      if(status != "")
      {
        ...
        for (i = 0; i < objects.length; i++) {
          ...
          {
            //detener la reproduccion del video
            video.stop();
